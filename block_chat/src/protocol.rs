...
// Why use two different sockets?
// The bootstrapping and the normal network operations utilize different ports
// the reason for that is twofold: separation of concerns and simplicity.
// Multiplexing every single message type (transaction broadcast, block broadcast,
// command, bootstrap message) on the same port is much more complicated than using two ports.
// Consider the following scenario:
// - the bootstrapping phase is almost over and the first node to receive the blockchain
//   from the bootstrap node wants to send a message
// - some other node who has not yet received the blockchain due to network latency
//   receives the message first
// What should the second node do? The message must not be lost, so some kind of
// buffer needs to be put in place just to prevent this scenario. If the message
// is sent to a different port instead, then as soon as the receiver node finishes
// bootstrapping, it will move on to the second port and deal with the message normally.
